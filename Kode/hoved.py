...
# Laster inn spiller- og zombiebilder
# convert_alpha() bevarer gjennomsiktigheten i PNG-filer
player_img_orig = pygame.image.load("Karakterer/player.png").convert_alpha()
player_img_orig = pygame.transform.scale(player_img_orig, (70, 70))

zombie_img_orig = pygame.image.load("Karakterer/zombie.png").convert_alpha()
zombie_img_orig = pygame.transform.scale(zombie_img_orig, (65, 65))

# Denne loader default fonten i str 36
...
enemies_to_spawn = 8  # Enemies_to_spawn setter hvor mange som spawner på første wave
...
# Spiller
player_pos = [WIDTH // 2, HEIGHT // 2]  # Posisjonen spilleren spawner på
...
# Her velger den en random del av kanten til skjermen. Hver fiende har x og y verdi, radius og speed.
def spawn_enemy():
...
# Denne kjører hver eneste frame og håndterer spawning og wave-systemet
def wave_update():
...
# Tegner hver zombie rotert slik at den alltid peker mot spilleren
def draw_enemies():
    for e in enemies:
        dx = player_pos[0] - e["x"]
        dy = player_pos[1] - e["y"]
        angle = -math.degrees(math.atan2(dy, dx))
        rotated = pygame.transform.rotate(zombie_img_orig, angle)
        rect = rotated.get_rect(center=(int(e["x"]), int(e["y"])))
        screen.blit(rotated, rect.topleft)

# Tegner spilleren rotert slik at den alltid peker mot musepekeren
def draw_player():
    mouse_x, mouse_y = pygame.mouse.get_pos()
    dx = mouse_x - player_pos[0]
    dy = mouse_y - player_pos[1]
    angle = -math.degrees(math.atan2(dy, dx)) + 180  # +180 snur spilleren 180 grader
    rotated = pygame.transform.rotate(player_img_orig, angle)
    rect = rotated.get_rect(center=(player_pos[0], player_pos[1]))
    screen.blit(rotated, rect.topleft)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if Meny == False and GameOver == False:
            # Skyter kule ved museklikk
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:  # Venstre klikk
    ...
    if Meny == False and GameOver == False:
        wave_update()                     # Håndterer spawning og wave-systemet
    ...
    if Meny == False and GameOver == False:
        ...
        draw_enemies()  # Zombie-PNGer, hver rotert mot spilleren
